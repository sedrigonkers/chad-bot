...
bot.onText(/\/suggest/, (message) => { //suggest post
  ...
  bot.on('message', (msg) => {
    ...
    return bot.sendMessage(adminId, 'Этот формат не поддерживается')
    ...
    case 'accept-suggestion':
      ...
      if (text) {
        return bot.editMessageText(`${text ? text : ''}\n\n*Принято ✅*`, options)
          .then(() => {
            return sendPostToAllUsers((...p) => bot.sendMessage(...p), text, { parse_mode: 'markdown' })
              .then(() => bot.sendMessage(chatId, '*Пост успешно опубликован 📝*', { parse_mode: 'markdown' })
              )
              .catch(err => bot.sendMessage('Произошла какая-то ошибка:\n\n', err))
          })
      }
...
async function sendPostToAllUsers(callback, file, options) {
  ...
  const send = () => {
    ...
    // bot.sendPhoto(users[i++], file, options)
